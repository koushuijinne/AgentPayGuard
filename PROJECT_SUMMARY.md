...
**AgentPayGuard** 是一个面向 AI Agent 的链上支付权限与风控系统，为 Kite Payment Track 黑客松开发。项目让 AI Agent 在执行链上稳定币支付时，具备**可验证身份、可编程权限、可审计记录、可阻断异常**的最小闭环。

### 核心特性
- 🤖 **AI Agent 核心**：支持自然语言支付请求解析和智能风险评估
- 🔒 **多层安全防护**：传统规则（白名单/限额） + AI风险评估 + 链上冻结检查
- 🚀 **端到端工作流**：从自然语言请求到链上执行的完整闭环
- 📊 **可验证的AI决策**：AI风险评估透明可解释，提供风险理由和建议
- 🔄 **优雅降级**：无AI API时自动使用回退解析器，保证系统可用性
...
根据代码分析（[`src/lib/ai-intent.ts`](src/lib/ai-intent.ts:44-120)），错误发生在 AI 意图解析时：

1. **配置了 `DEEPSEEK_API_KEY`**：系统检测到 DeepSeek API Key，自动选择 DeepSeek 作为提供商
2. **模型名称错误**：DeepSeek API 返回 "Model Not Exist" 错误，说明配置的模型名称不正确
3. **自动降级**：系统捕获错误后自动使用回退解析器（fallback parser），保证功能可用

### 解决方案

#### 方案 1：修正 DeepSeek 模型名称
检查 `.env` 文件中的 `AI_MODEL` 配置，确保使用正确的 DeepSeek 模型名称：
...
#### 方案 2：使用其他 AI 提供商
如果 DeepSeek 不可用，可以切换到其他提供商：

```bash
# 使用 Google Gemini（免费额度）
GEMINI_API_KEY=your-gemini-api-key-here
AI_MODEL=gemini-1.5-pro
...
系统会自动使用回退解析器，仍然可以正常工作（但不支持自然语言解析和风险评估）。

---

## 📋 Agent 工作约束

根据 [`.clinerules`](.clinerules) 文件，Agent 必须遵守以下约束：

### 安全约束
1. **禁止读取敏感文件**：拒绝读取任何 `.env*` 文件（含 `.env.local` 等）
2. **不修改安全政策**：不删除约束文件（除非用户明确要求）
3. **代码提交前必须验证**：
   - 通过 `pnpm typecheck`（0 errors）
   - 通过 `pnpm demo:pay` 和 `pnpm demo:reject`

### 工作流约束
4. **实时更新工作日志**：每个 Phase 完成后立即更新 [`AGENT_WORKLOG.md`](docs/internal/AGENT_WORKLOG.md)
5. **清晰的 Git 提交**：commit 含 Phase 号 + 操作描述
6. **每次修改后自动 push**：commit 完成后立即 `git push`
7. **任务追踪**：多步骤工作（3+ 步）使用 todo 工具

### 代码质量约束
8. **TypeScript 严格模式**：所有 `.ts` 通过 `pnpm typecheck`（0 errors）
9. **Demo 脚本必须工作**：`pnpm demo:pay`、`pnpm demo:reject` 必须正常
10. **不破坏现有功能**：重构时保证已通过测试仍通过

### 决策约束
11. **重要决定需确认**：架构变更、大重构、删除代码前询问用户
12. **文档控制**：创建新文档前询问是否真正需要
13. **设计决策文档化**：非平凡技术决策记录在适当文件中

### 项目结构约束
14. **不移动或删除核心文件**：不删除 [`src/lib/kite-aa.ts`](src/lib/kite-aa.ts)、[`policy.ts`](src/lib/policy.ts) 等
15. **环境变量范式**：所有配置有 [`.env.example`](.env.example) 条目

### 分支管理约束
16. **修改前检查分支**：每次修改文件前，检查主仓和子模块是否在 `main` 分支
    - 若主仓在 main：`git checkout -b feature/xxx` 切换到新分支
    - 若子模块在 main：`cd frontend && git checkout -b feature/xxx`
    - 详见 [`README.md`](README.md:80-120)「主仓 + 子模块都在新分支修改」章节

### 安全政策
- **禁止共享敏感信息**：不提交 `.env` 到 Git；不向 AI 或外部工具提供 `.env` 内容
- **AI Assistant 规则**：永不读取 `.env*` 文件（文件名黑名单）
...
- **Kite 白皮书**：[`resources/kite_whitepaper.pdf`](docs/resources/kite_whitepaper.pdf)
- **项目工作日志**：[`docs/internal/AGENT_WORKLOG.md`](docs/internal/AGENT_WORKLOG.md)（Phase 1-20 完整历史）

---

## ✅ 自检清单

- [ ] [`AGENT_WORKLOG.md`](docs/internal/AGENT_WORKLOG.md) 已更新
- [ ] `pnpm typecheck` 0 errors
- [ ] `pnpm demo:pay`、`pnpm demo:reject` 正常
- [ ] 未新增/修改 `.env*`
- [ ] commit message 清晰
- [ ] [`.clinerules`](.clinerules) 未被意外修改
- [ ] 主仓与子模块均不在 main 分支（否则已按 [`README.md`](README.md) 切换）
- [ ] 重要决定已文档化
