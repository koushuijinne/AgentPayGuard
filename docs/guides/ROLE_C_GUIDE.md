# Role C 指南：前端/编译与证据整合

本指南为 Role C (前端/编译负责人) 提供完整的工作流程，从 Role A 和 B 的交付物编译最终的 `for_judge.md`。
...
1. **收集 Tx Hash 证据** - 从 Role A (冻结) 和 Role B (支付) 获得真实的交易 Hash
2. **填充 for_judge.md** - 将 4 个赛道要求全部补齐，供评委判定
3. **制作演示资料** - 录制或截图 demo:pay / demo:reject，展示功能工作
4. **生成证据索引** - 汇总所有 Tx Hash、地址、链接供 Role D 使用
